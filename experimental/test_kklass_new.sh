#!/bin/bash
# Тесты для kklass_new.sh

source "$(dirname "${BASH_SOURCE[0]}")/kklass_new.sh"
...
echo ""
echo "=== Test 6: Проверка, что методы хранятся один раз ==="
echo "Количество Person методов:"
declare -p Person.getName 2>/dev/null | head -1
declare -p Person.getAge 2>/dev/null | head -1

echo ""
echo "Информация об объекте p1:"
