...
declare -gA _KK.script_dir_cache
declare -gA _KK.use_cache
declare -gA _KK.use_cache_mtime

kk.getScriptDir() {
    local source_file="${1:-${BASH_SOURCE[1]:-$0}}"
    
    if [[ -n "${_KK.script_dir_cache[$source_file]}" ]]; then
        echo "${_KK.script_dir_cache[$source_file]}"
        return 0
    fi
    
    local result="$(cd "$(dirname "$source_file")" && pwd)"

    _KK.script_dir_cache[$source_file]="$result"
    echo "$result"
}
...
kk.use() {
    ...
    if [[ $check_mtime -eq 1 ]]; then
        ...
        if [[ "${_KK.use_cache_mtime[$file]}" != "$mtime" ]]; then
            _KK.use_cache[$file]=1
            _KK.use_cache_mtime[$file]="$mtime"
            echo "$file"
            return 0
