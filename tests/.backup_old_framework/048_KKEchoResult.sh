#!/bin/bash
# 048_KKEchoResult.sh - Test automatic echo behavior of kk._return based on BASH_SUBSHELL
# (Previously tested explicit KK_ECHO_RESULT control, now only auto-detect via BASH_SUBSHELL)

source "$(dirname "${BASH_SOURCE[0]}")/common.sh"
parse_args "$@"

# Setup temp directory for tests
init_test_tmpdir "048"

# Setup: Create TestClass with function
defineClass "Calculator" "" \
    "property" "value" \
    "function" "add" 'RESULT=$((value + $1))'

Calculator.new calc
calc.value = "10"

# Test 1: Main shell - no echo (BASH_SUBSHELL == 0)
test_start "Main shell context: no echo output"
output_file="$TEST_TMP_DIR/output_1"
calc.add 5 >"$output_file" 2>&1
output=$(cat "$output_file")
rm "$output_file"

if [[ -z "$output" ]]; then
    test_pass "Main shell: no echo output"
else
    test_fail "Expected no output in main shell (got: '$output')"
fi

# Test 2: Subshell - auto echo (BASH_SUBSHELL > 0)
test_start "Subshell context: automatic echo output"
result=$(calc.add 3)
if [[ "$result" == "13" ]]; then
    test_pass "Subshell: automatic echo output"
else
    test_fail "Expected output '13' but got: '$result'"
fi

# Test 3: Nested subshell
test_start "Nested subshell context"
result=$(
    calc.add 2
)
if [[ "$result" == "12" ]]; then
    test_pass "Nested subshell: correct echo output"
else
    test_fail "Expected '12' in nested subshell (got: '$result')"
fi

# Test 4: Complex value with spaces in subshell
test_start "Subshell with values containing spaces"
defineClass "Concatenator" "" \
    "property" "prefix" \
    "function" "concat" 'RESULT="${prefix} $1"'

Concatenator.new concat_inst
concat_inst.prefix = "Hello"
result=$(concat_inst.concat "World")
if [[ "$result" == "Hello World" ]]; then
    test_pass "Subshell: spaces preserved in output"
else
    test_fail "Expected 'Hello World' but got: '$result'"
fi
concat_inst.delete

# Test 5: Empty value in subshell
test_start "Subshell with empty value"
defineClass "Empty" "" \
    "function" "getEmpty" 'RESULT=""'

Empty.new empty_inst
result=$(empty_inst.getEmpty)
if [[ -z "$result" ]]; then
    test_pass "Subshell: empty value echoes as empty"
else
    test_fail "Expected empty echo but got: '$result'"
fi
empty_inst.delete

# Cleanup
calc.delete
